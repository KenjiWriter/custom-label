                    ...
                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $shapes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shape): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="relative cursor-pointer group perspective-500">
                            ...
                            <div class="shape-card border-2 rounded-xl p-6 text-center transition-all duration-200"
                                 :class="$wire.selectedShape == <?php echo e($shape->id); ?> ? 'border-orange-500 bg-orange-100 shadow-lg selected' : 'border-gray-200 hover:border-gray-300 bg-orange-50'">
                                ...
                                <div class="shape-image-container w-20 h-20 mx-auto mb-4 bg-white rounded-lg shadow-sm border border-gray-100 flex items-center justify-center overflow-hidden">
                                    <!--[if BLOCK]><![endif]--><?php if($shape->icon_path): ?>
                                        <img src="<?php echo e(asset($shape->icon_path)); ?>"
                                             ...
                                             onerror="this.onerror=null; this.parentElement.innerHTML='<span class=\'text-orange-600 text-2xl font-bold icon-3d\'><?php echo e(substr($shape->name, 0, 1)); ?></span>';">
                                    <?php else: ?>
                                        ...
                                        <div class="w-20 h-20 bg-gradient-to-br from-orange-100 to-orange-200 rounded-xl flex items-center justify-center icon-3d">
                                            <span class="text-orange-600 text-2xl font-bold"><?php echo e(substr($shape->name, 0, 1)); ?></span>
                                        </div>
                                ...
                                <div class="shape-indicator absolute -top-2 -right-2 w-6 h-6 bg-orange-500 rounded-full flex items-center justify-center"
                                     x-show="$wire.selectedShape == <?php echo e($shape->id); ?>">
                    ...
                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $materials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $material): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <label class="relative cursor-pointer group perspective-500">
                            ...
                            <div class="material-card border-2 rounded-xl p-6 text-center transition-all duration-300"
                                 :class="$wire.selectedMaterial == <?php echo e($material->id); ?> ? 'border-orange-500 bg-orange-50 shadow-lg selected-item' : 'border-gray-200 hover:border-orange-300'">
                                ...
                                <div class="material-texture w-20 h-20 mx-auto mb-4 rounded-lg overflow-hidden border-2 border-gray-200 shadow-sm relative
                                          <?php echo e($isGlossy ? 'material-glossy' : ($isMatte ? 'material-matte' : '')); ?>">
                                    ...
                                    <?php else: ?>
                                        ...
                                        <div class="w-full h-full bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center">
                                            <span class="text-orange-600 text-xl font-bold icon-3d"><?php echo e(substr($material->name, 0, 2)); ?></span>
                                        </div>
                                    <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                </div>
                                ...
                                <!-- Indicator with animation -->
                                <div class="shape-indicator absolute -top-2 -right-2 w-6 h-6 bg-orange-500 rounded-full flex items-center justify-center"
                                     x-show="$wire.selectedMaterial == <?php echo e($material->id); ?>">
                                    <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                    </svg>
                                </div>
            ...
            <div x-show="currentStep === 3"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $laminateOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $laminate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <label class="relative cursor-pointer group perspective-500">
                                ...
                                <div class="laminate-card border-2 rounded-xl p-6 text-center transition-all duration-300 bg-orange-50"
                                     :class="$wire.selectedLaminate == <?php echo e($laminate->id); ?> ? 'border-orange-500 bg-orange-100 shadow-lg selected-item' : 'border-gray-200 hover:border-gray-300'">
                                    ...
                                    <div class="laminate-texture w-16 h-16 mx-auto mb-3 rounded-lg overflow-hidden border border-gray-200 relative">
                                        ...
                                        <?php else: ?>
                                            <div class="w-full h-full bg-gradient-to-br from-orange-100 to-orange-200 flex items-center justify-center">
                                                <span class="text-orange-600 text-sm font-medium icon-3d"><?php echo e(strtoupper(substr($laminate->finish_type ?? 'L', 0, 1))); ?></span>
                                            </div>
                                        <?php endif; ?><!--[if ENDBLOCK]><![endif]-->
                                        <!-- Efekty wizualne -->
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex flex-wrap gap-4 justify-center bg-orange-50 p-4 rounded-xl"
                         x-data="{ customSize: <?php if ((object) ('useCustomSize') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('useCustomSize'->value()); ?>')<?php echo e('useCustomSize'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('useCustomSize'); ?>')<?php endif; ?>.live }">
                    ...
                    <!-- Predefined Sizes -->
                    <!--[if BLOCK]><![endif]--><?php if(!$useCustomSize && $availableSizes->count() > 0): ?>
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4" wire:key="standard-sizes-<?php echo e($selectedShape); ?>">
                            <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $availableSizes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $size): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <label class="relative cursor-pointer group perspective-500">
                                    ...
                                    <div class="border-2 p-4 rounded-xl text-center transition-all duration-300 size-preview-container
                                        <?php echo e($selectedSize == $size->id
                                        ...
                                        <!-- Indicator with animation -->
                                        <!--[if BLOCK]><![endif]--><?php if($selectedSize == $size->id): ?>
                                            <div class="shape-indicator absolute -top-2 -right-2 w-6 h-6 bg-orange-500 rounded-full flex items-center justify-center">
                                                <svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                </svg>
...
unset($__errorArgs, $__bag); ?><!--[if ENDBLOCK]><![endif]-->
                    ...
                    <?php if(!$useCustomSize && $availableSizes->count() === 0 && $selectedShape): ?>
                        ...
                        <div class="flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-sm text-orange-700">
                                    <?php echo e($quantity); ?> szt. ×
                                    <?php echo e($quantity > 0 ? number_format($calculatedPrice / $quantity, 2) : '0.00'); ?> PLN
                                </p>
                            </div>
                            <div class="text-right">
                                <div class="text-3xl font-bold text-orange-900 transform transition-all duration-300 hover:scale-110"><?php echo e(number_format($calculatedPrice, 2)); ?> PLN</div>
