            ...
            <div x-show="currentStep === 2"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                    @foreach ($materials as $material)
                        <label class="relative cursor-pointer">
                            <input type="radio" wire:model.live="selectedMaterial" value="{{ $material->id }}"
                                class="sr-only peer">
                            <div
                                ...
                                <div
                                    ...
                                    @if ($material->image_path)
                                        <img src="{{ asset($material->image_path) }}" alt="{{ $material->name }}"
                    ...
                    @if (count($availableSizes) > 0)
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach ($availableSizes as $size)
                                <label class="relative cursor-pointer">
                                    <input type="radio" wire:model.live="selectedSize" value="{{ $size->id }}"
                                        class="sr-only peer">
                                    <div
                                        ...
                                        <div class="text-sm text-gray-500 mb-2">{{ $size->width_mm }} ×
                                            {{ $size->height_mm }} mm</div>

                                        <div class="relative h-20 bg-gray-100 rounded-lg overflow-hidden">
                                            <div class="absolute inset-0 flex items-center justify-center">
                                                <div style="width: {{ min(80, $size->width_mm / 2) }}px; height: {{ min(60, $size->height_mm / 2) }}px;"
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex flex-wrap justify-center gap-4 pb-2">
                        ...
                        @foreach ($laminateOptions as $option)
                            <label class="relative cursor-pointer flex-shrink-0"
                                style="min-width: 170px; max-width: 200px;">
                                <input type="radio" wire:model.live="selectedLaminate" value="{{ $option->id }}"
                                    class="sr-only peer">
                                <div class="p-3 border-2 rounded-xl transition-all duration-200 bg-white shadow-sm hover:shadow-md peer-checked:border-orange-600 peer-checked:shadow-lg peer-checked:bg-orange-50 h-full flex flex-col"
                                    style="min-height: 160px;">
                                    <div class="flex-shrink-0 mb-2 h-14 overflow-hidden rounded-lg relative">
                                        @if ($option->texture_image_path)
                                            <img src="{{ asset($option->texture_image_path) }}"
                                                alt="{{ $option->name }}" class="w-full h-full object-cover">
                                        @else
                                    ...
                                    <div class="flex-grow">
                                        ...
                                        <p class="text-xs text-gray-500">{{ Str::limit($option->description, 50) }}
                                        </p>
                                    </div>

                                    <div class="mt-2 text-center">
                                        <span
                                            class="text-xs font-medium text-orange-600">+{{ number_format(($option->price_multiplier - 1) * 100, 0) }}%</span>
                                    </div>
                                </div>
                            </label>
                        @endforeach
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                        @if ($selectedShape)
                            <div
                                ...
                                <span
                                    class="font-semibold text-gray-900">{{ $shapes->find($selectedShape)->name ?? 'Nie wybrano' }}</span>
                            </div>
                        @endif
                        ...
                        @if ($selectedMaterial)
                            <div
                                ...
                                <span
                                    class="font-semibold text-gray-900">{{ $materials->find($selectedMaterial)->name ?? 'Nie wybrano' }}</span>
                            </div>
                        @endif
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="flex items-center">
                                    <button type="button"
                                        wire:click="$set('quantity', Math.max(1, {{ $quantity }} - 10))"
                                    ...
                                    <button type="button" wire:click="$set('quantity', {{ $quantity }} + 10)"
                        ...
                        <div
                            ...
                            <div class="relative w-full h-full min-h-[200px] flex items-center justify-center">
                                <div class="absolute inset-0 bg-checkerboard flex items-center justify-center">
                                    ...
                                    @if ($tempArtworkPath)
                                        <img src="/storage/{{ $tempArtworkPath }}" alt="Podgląd grafiki"
                        ...
                        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200 text-center">
                            ...
                            <div class="text-3xl font-bold text-orange-600 mb-1">
                                {{ number_format($calculatedPrice, 2) }} zł</div>
                        ...
                        <div
                            ...
                            <button type="button" 
                                ...
                                @if(!$isConfigurationValid) disabled @endif>
                    ...
                    @if (config('app.debug'))
                        <div class="mt-8 p-4 bg-gray-100 rounded-xl text-sm text-gray-600">
                            ...
                            <div class="space-y-1">
                                ...
                                <div>Materiał: {{ $selectedMaterial ? 'ID: ' . $selectedMaterial : 'Nie wybrano' }}
                                </div>
                                <div>Rozmiar custom: {{ $useCustomSize ? 'TAK' : 'NIE' }}</div>
                                @if ($useCustomSize)
                                    <div>Wymiary: {{ $customWidth }}mm x {{ $customHeight }}mm</div>
                                @else
                                    <div>Wybrany rozmiar: {{ $selectedSize ? 'ID: ' . $selectedSize : 'Nie wybrano' }}
                                    </div>
